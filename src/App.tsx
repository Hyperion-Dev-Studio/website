...
function Button({ children, variant = "solid", size = "md", href, className = "" }: ButtonProps) {
  const Comp = (href ? "a" : "button") as any;
  return (
    <Comp href={href} className={`btn btn-${variant} btn-${size} ${className}`}>
      {children}
    </Comp>
  );
}

// Phone Mockup — reproduces provided screenshot closely (no Tailwind)
function PhoneMock() {
  return (
